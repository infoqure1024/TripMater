...
export class Odometer {
  ...
  add(cur: Fix): AddResult {
    ...
    if (cur.accuracy > c.maxAccuracyM) {
      return this.result('accuracy_gate', 0); // prev 据え置き
    }

    const p = this.prev;
    if (!p) {
      this.prev = cur;
      return this.result('first_fix', 0);
    }

    const dt = (cur.timestamp - p.timestamp) / 1000;
    if (dt <= 0) {
      return this.result('non_monotonic', 0); // prev 据え置き
    }
    if (dt > c.maxDtS) {
      this.prev = cur;
      return this.result('gap', 0); // 連続性リセット
    }
    ...
    if (speedAvailable) {
      ...
      // 低速はジッターに強い速度積分、それ以上は位置ベース
      const useSpeed = speed < c.lowSpeedMps;
      const d = useSpeed ? dSpeed : dPos;
      if (d > speed * dt * c.teleportFactor + 5) {
        return this.result('teleport', 0); // prev 据え置き(単発グリッチ対策)
      }
      this.totalMeters += d;
      this.prev = cur;
      return this.result(useSpeed ? 'counted_speed' : 'counted_position', d);
    }
    ...
    if (dPos > cur.accuracy * 0.5 && dPos < 100) {
      ...
      return this.result('counted_no_speed', dPos);
    }
    return this.result('no_speed_skip', 0);
  }

  private result(reason: AddReason, distanceAdded: number): AddResult {
    return { reason, distanceAdded, total: this.totalMeters };
  }
}
